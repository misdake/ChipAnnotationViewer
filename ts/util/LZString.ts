export class LZString {
    ...
    private static baseReverseDic: { [key: string]: { [key: string]: number } } = {};

    static getBaseValue(alphabet: string, character: string) {
    ...
    static compressToBase64(input: string) {
        if (input == null) return "";
        let res = LZString._compress(input, 6, function (a: number) {
    ...
    static decompressFromBase64(input: string) {
        if (input == null) return "";
    ...
    static compressToUTF16(input: string) {
        if (input == null) return "";
        return LZString._compress(input, 15, function (a: number) {
            return String.fromCharCode(a + 32);
        }) + " ";
    }

    static decompressFromUTF16(compressed: string) {
        if (compressed == null) return "";
        if (compressed == "") return null;
        return LZString._decompress(compressed.length, 16384, function (index: number) {
            return compressed.charCodeAt(index) - 32;
        });
    }

    //compress into uint8array (UCS-2 big endian format)
    static compressToUint8Array(uncompressed: string) {
    ...
    static decompressFromUint8Array(compressed: string) {
    ...
    static compressToEncodedURIComponent(input: string) {
        if (input == null) return "";
        return LZString._compress(input, 6, function (a: number) {
            return LZString.keyStrUriSafe.charAt(a);
        });
    }

    //decompress from an output of compressToEncodedURIComponent
    static decompressFromEncodedURIComponent(input: string) {
        if (input == null) return "";
        if (input == "") return null;
    ...
    static _compress(uncompressed: string, bitsPerChar: number, getCharFromInt: (a: number) => string) {
